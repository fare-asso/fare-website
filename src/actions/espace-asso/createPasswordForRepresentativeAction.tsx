...
export default async function createPasswordForRepresentativeAction(prevState: {error?: string, success?: boolean} | undefined, formData: FormData) {
    ...
    if(password.length < 8) {
        return { error : "La longueur du mot de passe doit être d'au moins 8 caractères"}
    }

    // password and passwordConf are not identical
    if(password != passwordConf) {
        return { error : "Le mot de passe et la confirmation du mot de passe ne sont pas identiques"}
    }
    ...
    if(error) {
        return { error : "Echec de la création du mot de passe, veuillez contacter un administrateur"}
    }
