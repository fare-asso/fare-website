...
export const downloadFolderAction = createServerFn({ method: "POST" })
    .validator((data: string) => data)
    .handler(
        withServerAction(
            "downloadFolderAction",
            async ({ data: folderPath }): Promise<ActionState> => {
                ...
                const pdf = await tryCatch(
                    generateAdhesionPdfFromRecord(adhesion.value)
                )
                if (!pdf.success) {
                ...
                const slug =
                    sanitizeString(adhesion.value.sigle) ||
                    `adhesion-${adhesion.value.id}`
