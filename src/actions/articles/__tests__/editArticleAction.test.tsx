...
describe("editArticleAction", () => {
    it("requires authentication", async () => {
        ...
        expect(await editArticleAction({ data: fd() })).toEqual({
    ...
    it("requires the edit:article permission", async () => {
        ...
        const res = await editArticleAction({ data: fd() })
        expect(res.error).toMatch(/permission/)
        expect(h.update).not.toHaveBeenCalled()
    })

    it("rejects a non-numeric id", async () => {
        const res = await editArticleAction({ data: fd({ id: "abc" }) })
        expect(res).toEqual({ error: "L'id de l'article est eronné" })
    })

    it("rejects a payload missing required fields", async () => {
        const res = await editArticleAction({ data: fd({ title: "" }) })
        expect(res).toEqual({
