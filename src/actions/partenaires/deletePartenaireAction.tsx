...
async function deletePartenaireActionImpl(
  ...
  if (partenaire.logoPath.length > 0) {
    const { error } = await supabase.storage.from("partner-pictures").remove([partenaire.logoPath]);
...
export default withServerAction("deletePartenaireAction", deletePartenaireActionImpl);
