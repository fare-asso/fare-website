...
import { bulkUpdateRoleAction } from "../bulkUpdateRole"
...
describe("bulkUpdateRole", () => {
    it("requires authentication", async () => {
        ...
        expect(
            await bulkUpdateRoleAction({
                data: { userIds: ["u2"], newRole: "MEMBER" }
            })
        ).toEqual({
            success: false,
    ...
    it("rejects when only the current user would change", async () => {
        const res = await bulkUpdateRoleAction({
            data: { userIds: ["user-1"], newRole: "MEMBER" }
        })
    ...
    it("updates the role for filtered users and revalidates", async () => {
        const res = await bulkUpdateRoleAction({
            data: { userIds: ["u2", "user-1"], newRole: "ASSO_OWNER" }
        })
        expect(res).toEqual({
            success: true,
            updatedCount: 1,
