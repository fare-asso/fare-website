...
export const submitTutorApplicationAction = createServerFn({ method: "POST" })
    .validator((data: FormData) => data)
    .handler(
        withServerAction(
            "submitTutorApplication",
            async ({ data: formData }): Promise<ActionResponse> => {
                ...
                const firstInitial =
                    parsedData.firstName.toLowerCase().at(0) ?? ""
                const sanitizedName =
                ...
                const { data: cvUploadData, error: cvUploadError } =
                    await supabase.storage
                        .from("btp-tutor-application")
                        .upload(
                            `${folderName}/cv-${sanitizedName}.pdf`,
                            parsedData.cv
                        )
                if (cvUploadError) {
                    return {
                ...
                const { data: lmUploadData, error: lmUploadError } =
                    await supabase.storage
                        .from("btp-tutor-application")
                        .upload(
                ...
                // Email is best-effort: the application has already been
                // persisted.
                await sendEmail({
