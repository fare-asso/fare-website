...
export const downloadTutorApplicationsZipAction = createServerFn({
    method: "POST"
})
    .validator((data: TDownloadTutorApplications) => data)
    .handler(
        withServerAction(
            "downloadTutorApplicationsZipAction",
            async ({ data: ids }): Promise<DownloadResult> => {
                ...
                const downloadApplication = async (
                    app: BTPTutorApplication
                ) => {
                    const folder = `${sanitizeString(
                        app.lastName
                    )}-${sanitizeString(app.firstName)}-${app.id}`
                    const files = [
                ...
                // Download in small batches so we never open ~150 concurrent
                // storage requests on a large selection (75 apps × 2 files).
                const DOWNLOAD_CONCURRENCY = 10
                const downloaded: Awaited<
                    ReturnType<typeof downloadApplication>
                >[] = []
                for (
                    let i = 0;
                    i < applications.value.length;
                    i += DOWNLOAD_CONCURRENCY
                ) {
                    const batch = applications.value.slice(
                        i,
                        i + DOWNLOAD_CONCURRENCY
                    )
                    // oxlint-disable-next-line no-await-in-loop -- sequential batches cap concurrency
                    const results = await Promise.all(
                        batch.map(downloadApplication)
                    )
                ...
                // The whole archive is returned as base64 through the server
                // action. The 75-candidature cap (enforced by the schema)
                // bounds memory/response size; switch to a streamed Route
                // Handler if individual PDFs grow large.
                return {
                    success: true,
