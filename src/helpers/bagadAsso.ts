...
interface DetailedEquipment extends Equipement, Pick<BagadAssoEquipment, 'deposit' | 'name' | 'imagePath'> {}

type TicketWithDetailedEquipments = Omit<BagadAssoTicket, 'equipments'> & {
    equipments: DetailedEquipment[];
};

export async function computeTotalDeposit(ticket: BagadAssoTicket) : Promise<number> {

    const equipments: Equipement[] = await JSON.parse(ticket.equipments!.toString())
    ...
    const depositMap = equipmentData.reduce((map, equipment) => {
...
export async function joinTicketAndEquipment(ticket: BagadAssoTicket): Promise<BagadAssoTicket> {
    const equipments: Equipement[] = await JSON.parse(ticket.equipments!.toString());
    const equipmentIds: number[] = equipments.map((e) => e.id);
