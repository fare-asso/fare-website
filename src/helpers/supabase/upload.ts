...
export async function uploadFile(bucket: string, folder: string | undefined, file: File , name: string, maxSizeInMb: number, acceptedExtensions: string[]) : Promise<Response> {
    ...
    const fileName = name ? `${sanitizeString(name)}.${fileExt}` : `${Math.random().toString(36).slice(2)}.${fileExt}`;
