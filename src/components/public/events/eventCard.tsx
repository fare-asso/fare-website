...
export default function EventCard({event, archive, imageUrl} : {event: Event, archive: boolean, imageUrl: string}) {
    ...
    const location: { json?: JsonLocation, string?: string} = processLocationData(event.location);

    return (
        <div className={`h-32 lg:h-44 rounded-xl w-full md:w-2/3 lg:w-1/2 p-2 flex flex-row`} style={{backgroundColor}}>
            <Image src={imageUrl} width={600} height={400} alt={`Photo de l'évènement ${event.name}`}
            ...
            <div className="w-2/3 md:w-1/4 flex flex-col items-start pl-2 mr-4">
                ...
                {/* Location */}
                <div className="flex flex-row items-end w-full justify-start" style={{color: fontColor}}>
                    <MdLocationPin size={20} className="min-w-4 min-h-4"/>
                    <span className="text-nowrap text-ellipsis overflow-hidden text-sm mt-1">{location.json ? location.json.displayName.split(',')[0] : location.string!.split(',')[0] }</span>
                </div>
            </div>
