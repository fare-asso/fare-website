    ...
    return (
        <form onSubmit={handleSubmit} className={`w-full lg:w-[60%] flex flex-col items-start
            ...
            <div className="mt-4">
                ...
                <option value="display" title="Bugs d'affichage - Problèmes d'affichage sur différentes tailles d'écran ou navigateurs, mauvais positionnement d'éléments, texte qui se chevauche, images qui ne s'affichent pas correctement, etc.">Affichage</option>
                <option value="interaction" title="Bugs d'interaction - Boutons ou liens qui ne fonctionnent pas, formulaires qui ne se soumettent pas correctement, erreurs lors de la navigation, etc.">Interaction</option>
                <option value="functionality" title="Bugs de fonctionnalité - Fonctionnalités qui ne fonctionnent pas comme prévu, erreurs de calcul, données qui ne s'enregistrent pas correctement, etc.">Fonctionnalité</option>
                <option value="security" title="Bugs de sécurité - Failles de sécurité telles que failles XSS (Cross-Site Scripting), CSRF (Cross-Site Request Forgery), injection SQL, etc.">Sécurité</option>
                <option value="performance" title="Bugs de performance - Lenteurs dans le chargement de la page, ralentissements lors de l'utilisation de certaines fonctionnalités, etc.">Performance</option>
                <option value="compatibility" title="Bugs de compatibilité - Problèmes d'affichage ou de fonctionnalité sur certains navigateurs ou appareils spécifiques.">Compatibilité</option>
                <option value="accessibility" title="Bugs d'accessibilité - Problèmes pour les utilisateurs ayant des besoins spéciaux (malvoyants, malentendants, etc.)">Accessibilité</option>
                <option value="i18n" title="Bugs d'internationalisation - Traductions incorrectes, problèmes d'affichage pour certaines langues, etc.">Internationalisation</option>
                <option value="data" title="Bugs de données - Erreurs dans l'affichage, le stockage ou la manipulation des données.">Données</option>
                <option value="documentation" title="Bugs de documentation - Informations manquantes, incorrectes ou déroutantes dans la documentation du site.">Documentation</option>
                <option value="other">Autre</option>
                </select>

                <p className="p-2 text-sm opacity-85">
                Sélectionnez le type de bug rencontré. Survolez les options avec la souris pour voir la description détaillée.
                </p>
            </div>
            ...
            <div>
                ...
                <div className="w-full relative">
                    <textarea name="description" id="description" rows={5} maxLength={maxCharactersLength}
                    className="w-full" onChange={handleDescriptionChange}
                    ...
                    />
                    <span style={{color: charactersLength == maxCharactersLength ? 'red' : 'black'}} className="absolute bottom-0 right-0 m-2 mr-4 p-1 rounded-lg bg-white text-sm opacity-80 select-none">
