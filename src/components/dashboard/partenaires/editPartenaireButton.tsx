...
export default function EditPartenaireButton({ partenaire }: { partenaire: Partenaire }) {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="h-[90%] max-h-[90%] sm:max-w-[60%] lg:max-w-[40%]">
        <DialogHeader>
          ...
          <DialogDescription>Formulaire de modification du partenaire.</DialogDescription>
        </DialogHeader>
              ...
              children={(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    <FieldLabel htmlFor={field.name}>Nom du partenaire</FieldLabel>
                    <Input
                      id={field.name}
                      name={field.name}
                      ...
                      placeholder="Nom"
                    />
                    {isInvalid && <FieldError>Le nom est requis.</FieldError>}
                  </Field>
                );
              }}
            />
              ...
              children={(field) => {
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    ...
                    />
                    {isInvalid && (
                      <FieldError>La description est requise (max 1000 caractères).</FieldError>
                    )}
                  </Field>
                );
              }}
                ...
                return (
                  <Field data-invalid={isInvalid}>
                    <FieldLabel htmlFor={field.name}>
                      Logo <span className="text-muted-foreground">(optionnel)</span>
                    </FieldLabel>
                    <FieldDescription>
                      Laissez vide pour conserver le logo actuel. Format : PNG, JPG, WebP, SVG.
        ...
        <DialogFooter>
          <Button type="submit" form="editPartenaireForm" disabled={isPending}>
            {isPending ? <Loader2Icon className="animate-spin" /> : null} Valider les modifications
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
