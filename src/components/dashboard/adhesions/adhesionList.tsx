...
export default async function AdhesionList() {
    ...
    if(!adhesions) {
        return(
            <span className="text-xl text-red-800">Echec du chargement des adhésions, veuillez réessayer</span>
        )
    } else {

        const adhesionCards: JSX.Element[] = adhesions.map((adhesion) => <AdhesionCard key={adhesion.id} adhesion={adhesion} />)
