...
export default function ArticleCard({article} : {article: Article}) {
    ...
    const [formState, formAction] = useFormState<{error?: string, success?: boolean} | undefined, number>(deleteArticleAction, undefined)
        ...
        <div className="w-full rounded-lg border bg-card text-card-foreground shadow-sm px-4 py-4 h-16 flex flex-row items-center justify-between">
            ...
            <div id="buttons" className="flex flex-row items-center">
                <Button variant={"default"} className='px-3 mr-2 hidden md:block'>
                    <Link href={`/actualites/articles/${article.id}`} className=''>
                        <MdVisibility size={17}/>
                    </Link>
                </Button>
                
                <EditArticleButton className="mr-2 px-2 py-2 sm:px-4" article={article}>
                    <MdEdit size={20} className="mr-0 sm:mr-1"/>
                    <div className="hidden sm:flex">Modifier</div>
                </EditArticleButton>
                <Button variant="destructive" className="px-2 py-2 sm:px-4" onClick={handleDelete} disabled={isLoading}>
                    { isLoading ? <LoadingRing /> : <MdDelete size={20} className="mr-0 sm:mr-1"/> }
